...
from app.api.dependencies import ShipmentServiceDep, SessionDep
...
async def get_latest_shipment(session: ShipmentServiceDep):
    return await session.get_latest()


# # recuperer le shipement selon id  ?
@router.get("/{id}", response_model=ShipementRead)
async def get_shipment(id: int, session: ShipmentServiceDep):
...
async def get_shipment_field(
    id: int, field: str, session: ShipmentServiceDep
) -> dict[str, Any]:
...
async def update_shipment(
    id: int, shipment_update: ShipementUpdate, session: ShipmentServiceDep
) -> Shipment:
...
# # #effacer un shipment
@router.delete("/{id}")
async def delete_shipment(id: int, session: ShipmentServiceDep) -> dict[str, Any]:
